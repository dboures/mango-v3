...
impl Processor {
    ...
    pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> MangoResult {
        ...
        match instruction {
            ...
            MangoInstruction::EditSpotOrder { order } => {
                msg!("Mango: EditSpotOrder");
                Self::edit_spot_order(program_id, accounts, order)
            } 
            MangoInstruction::EditSpotOrder2 { cancel_order, new_order } => {
                msg!("Mango: EditSpotOrder2");
                let cancel_data = serum_dex::instruction::MarketInstruction::CancelOrderV2(cancel_order).pack();
                Self::edit_spot_order2(program_id, accounts, cancel_data,  new_order)
            } 
...
fn invoke_replace_order<'a>(
    ...
    let data = serum_dex::instruction::MarketInstruction::ReplaceOrderByClientId(order).pack();
    let mut instruction = Instruction {
        program_id: *dex_prog_ai.key,
        data,
        accounts: vec![
