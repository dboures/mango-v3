...
pub async fn edit_spot_order_scenario(
    test: &mut MangoProgramTest,
    mango_group_cookie: &mut MangoGroupCookie,
    spot_order: &(usize, usize, serum_dex::matching::Side, f64, f64, u64),
) {
    mango_group_cookie.run_keeper(test).await;

    let (user_index, market_index, order_side, order_size, order_price, client_order_id) = *spot_order;
    let mut spot_market_cookie = mango_group_cookie.spot_markets[market_index];
    spot_market_cookie
        .edit_order(test, mango_group_cookie, user_index, order_side, order_size, order_price, client_order_id)
        .await;
...
#[allow(dead_code)]
pub async fn edit_spot_order_scenario2(
    test: &mut MangoProgramTest,
    mango_group_cookie: &mut MangoGroupCookie,
    ...
    let mut spot_market_cookie = mango_group_cookie.spot_markets[market_index];
    spot_market_cookie
        .edit_order2(test, mango_group_cookie, user_index, order_side, order_size, order_price, cancel_order_id)
        .await;
